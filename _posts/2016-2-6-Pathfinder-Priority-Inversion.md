...
Not all threads are created equal. Some threads are more important than others, and hence are assigned a higher priority. A thread scheduler will do its best to schedule higher priority threads first. (If you are one of my less technologically inclined readers, you can think of a thread as a task that runs on a computer processor).
